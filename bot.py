        ...
        if text.isdigit() and len(text) > 5:            # скорее всего Steam‑ID
            ...
            if isinstance(result, int):
                ...
                await update.message.reply_text(
                    ...
                    "Нужна помощь? Нажмите кнопку «❓ Как найти Steam ID»",
                    reply_markup=steam_help_keyboard()
                )
            elif result == "no_api_key":
                await update.message.reply_text(
                    "⚠️ Сервис временно недоступен. Введите часы вручную."
                )
                context.user_data["step"] = "hours_manual"
            elif result == "no_game":
                await update.message.reply_text(
                    "⚠️ Игра Rust не найдена в вашем аккаунте. Введите часы вручную."
                )
                context.user_data["step"] = "hours_manual"
            else:
                await update.message.reply_text(
