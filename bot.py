...
# ───────────────────────────────────────
DB_NAME = "users.db"
REQUIRED_CHANNEL = "@rustycave"          # канал, на который надо подписаться
STEAM_API_KEY = "YOUR_STEAM_API_KEY"    # ← ваш ключ Steam (можно оставить пустым)

# **ID администратора** – только ваш
ADMIN_IDS = {904487148}
...
# ───────────────────────────────────────
def save_user(tg_id, name, hours, age, bio, username, is_active=1, is_verified=0):
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute(
        ...
        logger.error(f"Database error in save_user: {e}")
    finally:
        if conn:
            conn.close()
...
def get_all_active_partners(exclude_id):
    ...
    try:
        ...
        logger.error(f"Database error in get_all_active_partners: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def add_like(from_id, to_id):
    try:
        ...
        logger.error(f"Database error in add_like: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def add_pending_like(from_id, to_id, from_name):
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute(
        ...
        logger.error(f"Database error in add_pending_like: {e}")
    finally:
        if conn:
            conn.close()


def get_pending_likes(to_id):
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute(
        ...
        logger.error(f"Database error in get_pending_likes: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def remove_pending_like(from_id, to_id):
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute(
        ...
        logger.error(f"Database error in remove_pending_like: {e}")
    finally:
        if conn:
            conn.close()
...
def add_report(reporter_id, reported_id):
    try:
        ...
        logger.error(f"Database error in add_report: {e}")
    finally:
        if conn:
            conn.close()
...
def activate_user(tg_id):
    try:
        ...
        logger.error(f"Database error in activate_user: {e}")
    finally:
        if conn:
            conn.close()
...
def block_user(tg_id):
    ...
    try:
        ...
        logger.error(f"Database error in block_user: {e}")
    finally:
        if conn:
            conn.close()
...
def unblock_user(tg_id):
    ...
    try:
        ...
        logger.error(f"Database error in unblock_user: {e}")
    finally:
        if conn:
            conn.close()
...
def get_blocked_list():
    ...
    try:
        ...
        logger.error(f"Database error in get_blocked_list: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def clear_reports_for(user_id):
    ...
    try:
        ...
        logger.error(f"Database error in clear_reports_for: {e}")
    finally:
        if conn:
            conn.close()
...
def get_reports_summary():
    ...
    try:
        ...
        logger.error(f"Database error in get_reports_summary: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def update_stat(user_id, field):
    ...
    if field not in allowed_fields:
        logger.error(f"Invalid field for update_stat: {field}")
        return
    ...
    try:
        ...
        logger.error(f"Database error in update_stat: {e}")
    finally:
        if conn:
            conn.close()
...
def validate_steam_id(steam_id):
    ...
    try:
        ...
        # Steam ID обычно находится в диапазоне 76561197960265728 + ...
...
def steam_instructions_keyboard():
    return InlineKeyboardMarkup([
...
async def ask_to_subscribe(update: Update):
    await update.message.reply_text(
        f"⚠️ Подпишитесь на канал {REQUIRED_CHANNEL} и нажмите /start",
        reply_markup=None,
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• 🚨 *Пожаловаться* — в любой анкете.\n\n"
        "🔧 **Команды для администратора** (только для ID 904487148):\n"
        "`/reports` — список анкет с жалобами, кнопки «Заблокировать»/«Снять жалобы».\n"
        "`/block <id>` — вручную заблокировать пользователя.\n"
        "`/unblock <id>` — снять блокировку.\n"
        "`/blocked` — увидеть всех заблокированных.\n",
        parse_mode="Markdown",
...
async def handle_text_and_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "choose_method":
        # Пользователь нажал кнопку «Ввести часы вручную» или «Привязать Steam»
        if text.isdigit() and len(text) > 5:            # скорее всего Steam‑ID
            hours = verify_user_steam(user.id, text)
            if isinstance(hours, int):
                await update.message.reply_text(
                    f"✅ Получено {hours} часов из Steam.\n"
                    "Теперь укажите ваш возраст:",
                )
                context.user_data["hours"] = hours
                context.user_data["step"] = "age"
            elif hours == "invalid_id":
                await update.message.reply_text(
                    "⚠️ Неверный формат Steam ID. Попробуйте ещё раз или нажмите «Ввести часы вручную».\n\n"
                    ...
                    "Копируем: 76561198000000000",
                    parse_mode="Markdown",
                    reply_markup=steam_instructions_keyboard()
                )
            else:
                await update.message.reply_text(
            ...
            # пользователь ввёл «часы вручную» (не число или слишком коротко)
            await update.message.reply_text(
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- ЖАЛОБА НА ПОЛЬЗОВАТЕЛЯ ----------
        ...
        # Уведомляем администратора о новой жалобе
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(
                    admin_id,
                    f"⚠️ Новая жалоба на пользователя {reported_id} от {reporter_id}"
                )
            except Exception:
                pass
    ...
    # ---------- ПОМОЩЬ ПО STEAM ──
        ...
        await query.edit_message_text(
            ...
            "Пример: https://steamcommunity.com/profiles/76561198000000000\n"
            "Копируем: 76561198000000000\n\n"
            "💡 Совет: Убедитесь, что ваш профиль открыт для просмотра",
            parse_mode="Markdown"
        )
...
# ── УВЕДОМЛЕНИЯ О НОВЫХ ПОЛЬЗОВАТЕЛЯХ ──
async def notify_about_new_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    profile = get_user_profile(user.id)
    if not profile:
        return
    hours, age = profile[1], profile[2]

    # (функция get_new_users_for_user уже была в предыдущем коде)
    # Для простоты будем использовать обычный поиск в базе
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute(
            """
            SELECT telegram_id, name, hours, age, bio, username, is_verified
            FROM users
            WHERE telegram_id != ?
              AND is_active = 1
              AND ABS(hours - ?) < 200
        ...
        logger.error(f"Database error in notify_about_new_users: {e}")
        return
    finally:
        if conn:
            conn.close()
    ...
    for nu in new_users:
        nu_id, name, nu_hours, nu_age, bio, username, is_verified = nu
        # Помечаем, что уже уведомили, чтобы не спамить каждый раз
        try:
            conn = sqlite3.connect(DB_NAME)
            cur = conn.cursor()
            cur.execute(
                "INSERT OR IGNORE INTO new_user_notifications (user_id, notified_new_id) VALUES (?, ?)",
                (user.id, nu_id)
            )
            conn.commit()
        except sqlite3.Error as e:
            logger.error(f"Database error in marking notification: {e}")
        finally:
            if conn:
                conn.close()
        ...
        verified_badge = "✅" if is_verified else ""
        kb = [
            [InlineKeyboardButton("❤️ Лайк", callback_data=f"like_{nu_id}")],
            [InlineKeyboardButton("👎 Дизлайк", callback_data=f"dislike_{nu_id}")],
        ]
        await context.bot.send_message(
            chat_id=user.id,
            text=(
                f"🎉 *Новый пользователь {verified_badge}*\n\n"
                f"📛 {name}\n"
                f"⏰ Часы: {nu_hours}\n"
                f"🎂 Возраст: {nu_age}\n"
                f"💬 {bio}\n"
                f"🔗 @{username if username else 'не указано'}"
            ),
            parse_mode="Markdown",
...
async def reports_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for reported_id, cnt in reports:
        # Получаем краткую инфу о пользователе
        ...
        # Кнопки «Заблокировать» и «Снять жалобы»
